...
const GroupChat = () => {
    ...
    const fetchChannel = async () => {
        try {
          const response = await fetch("http://194.233.93.124:3030/teman/discordChannels");
          if (!response.ok) {
    ...
    return (
        <div className="mb-5 text-left">
            ...
            <Button className="bg-[#38B0AB] hover:bg-teal-700 text-gray-100 px-4 py-2 rounded-md flex items-center" onClick={console/log()}>
            ...
            {channels.map((channel) => (
                <Link to={`/groupchat/${encodeURIComponent(channel.channel_name)}`}
                state={{ channel_id: channel.channel_id, server_id: channel.server_id }} className="inline-flex items-center justify-center p-4 space-x-4 shadow-md rounded-lg overflow-hidden text-base font-medium text-gray-900 rounded-lg bg-[#F5F7F9] hover:text-[#B23E19] hover:bg-[#F9A682]">
